...
(function () {
  ...
  async function loadPlayersList() {
    ...
    try {
      ...
      console.log(`Found ${relationshipsSnapshot.docs.length} player relationships`);
          ...
          if (relationship.status === 'pending' && !relationship.playerId) {
            ...
            if (playerDoc.exists()) {
              ...
              playersData.push({
                ...
                team: 'General',
                status:
                  relationship.status === 'accepted' ? 'Active' : relationship.status,
                relationshipId: relationshipDoc.id,
  ...
  async function handleAddPlayer(playerData) {
    ...
    try {
      ...
      if (!existingSnapshot.empty) {
        hideLoadingSpinner('playersList');
        if (typeof showToast === 'function') {
          showToast('A player with this email is already in your list.', 'error');
        }
        return;
      }
      ...
      if (!userSnapshot.empty) {
        ...
        if (playerUserData.role === 'player') {
          ...
          hideLoadingSpinner('playersList');
          if (typeof showToast === 'function') {
            showToast('This email belongs to a coach account, not a player.', 'error');
          }
          return;
        }
      }
      ...
      try {
        ...
        const docRef = await addDoc(collection(db, 'coach_players'), relationshipData);
  ...
  async function handleRemovePlayer(playerId) {
    ...
    try {
      ...
      try {
        ...
        console.warn("Could not update player's coachId (player may not exist):", error);
      ...
      showSuccessMessage(`${playerName} has been removed from your player list.`);
    } catch (error) {
  ...
  function setupEventListeners() {
    ...
    const closeRemovePlayerModal1 = document.getElementById('closeRemovePlayerModal');
    const cancelRemovePlayer = document.getElementById('cancelRemovePlayer');
    const removePlayerBackdrop = document.getElementById('removePlayerBackdrop');
