...
import { getTodayGMT8 } from './timezone-utils.js';

// (function () {
/**
 * Clean schedule data for Firestore storage
 * Converts undefined/null values to empty strings and trims whitespace
...
/**
 * Open a modal dialog
 * Shows modal with animation and sets default date values
 * @param {string} modalId - DOM ID of the modal element to open
 * @returns {void}
 */
function openModal(modalId) {
  ...
  if (modal && backdrop) {
    ...
    // Set default date to today (GMT+8) for date inputs in the modal
    const dateInputs = modal.querySelectorAll('input[type="date"]');
    dateInputs.forEach((input) => {
      if (!input.value) {
        input.value = getTodayGMT8();
      }
    });
 ...
 * Handles animation and restores page scroll
 * @returns {void}
 */
function closeModal() {
 ...
 * Uses toast notification system if available, falls back to basic implementation
...
export function openLogTrainingModal() {
  ...
  if (!window.editingSessionId) {
    ...
    setTimeout(() => {
      ...
      const submitBtn = document.querySelector('#logTrainingForm button[type="submit"]');
      if (submitBtn) {
...
window.openLogTrainingModal = openLogTrainingModal;

export function openRecordMatchModal() {
  ...
  if (!window.editingMatchId) {
    ...
    setTimeout(() => {
      ...
      const submitBtn = document.querySelector('#recordMatchForm button[type="submit"]');
      if (submitBtn) {
...
window.openRecordMatchModal = openRecordMatchModal;

export function openSetGoalModal() {
  ...
  if (!window.editingGoalId) {
    ...
    setTimeout(() => {
      ...
      const submitBtn = document.querySelector('#setGoalForm button[type="submit"]');
      if (submitBtn) {
...
async function handleTrainingSubmit(event) {
  ...
  if (!authState.authenticated) {
    if (typeof showToast === 'function') {
      showToast('Error: You must be logged in to log training sessions.', 'error');
    } else {
  ...
  if (userRole === 'coach') {
    ...
    coachId = authState.userData?.coachId || null;
  }
  ...
  try {
    if (window.editingSessionId) {
      ...
      console.log('Training session updated in Firestore:', window.editingSessionId);
...
async function handleMatchSubmit(event) {
  ...
  // Parse score string "21-15, 21-18"
  const scoreString = formData.get('matchScore') || '';
  ...
  if (sets[1]) {
    const parts = sets[1].split('-').map(Number);
    if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
      yourScore2 = parts[0];
      opponentScore2 = parts[1];
    }
  }
  if (sets[2]) {
    const parts = sets[2].split('-').map(Number);
    if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
      yourScore3 = parts[0];
      opponentScore3 = parts[1];
    }
  }
  ...
  const rawData = {
    ...
    matchType: formData.get('matchType') || 'Singles',
    result: formData.get('matchResult'),
    yourScore: yourScore,
    opponentScore: opponentScore,
    yourScore2: yourScore2,
    opponentScore2: opponentScore2,
    yourScore3: yourScore3,
    opponentScore3: opponentScore3,
...
async function handleEventSubmit(event) {
  ...
  if (submitButton) {
    ...
    submitButton.textContent = window.editingEventId ? 'Updating...' : 'Adding...';
  }
...
function initializeModals() {
  ...
  if (trainingForm) {
    trainingForm.removeEventListener('submit', handleTrainingSubmit);
    trainingForm.addEventListener('submit', handleTrainingSubmit);
  }

  const matchForm = document.getElementById('recordMatchForm');
  if (matchForm) {
    matchForm.removeEventListener('submit', handleMatchSubmit);
