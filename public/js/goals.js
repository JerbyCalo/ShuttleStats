...
(function () {
   ...
   * Validates and sanitizes all goal fields before database operations
   * @param {Object} data - Raw goal data from form or API
   * @returns {Object} Cleaned goal data safe for Firestore
   */
   ...
   * @returns {string} Formatted date string with month, day, and year
   */
  ...
  /**
   * Get CSS class for status badge based on goal status
   * @param {string} status - Goal status ('Completed', 'In Progress', 'Not Started', etc.)
   ...
   * @param {string} priority - Goal priority ('High', 'Medium', 'Low')
   * @returns {string} CSS class name for the priority badge
   */
   ...
   * Returns formatted string with days count or status message
   * @param {string} targetDate - Target date string in YYYY-MM-DD format
   * @returns {string} Formatted string (e.g., "5 days left", "Due today", "3 days overdue")
   */
    ...
    card.innerHTML = `
      ...
      <div class="goal-card-header">
        ...
        <div class="goal-status">
          <span class="status-badge ${getStatusClass(goal.status)}">${goal.status}</span>
        </div>
      </div>
  ...
  async function renderGoals(goalsData = null) {
    ...
    try {
      ...
      // Show user-friendly error message
      let errorMessage = 'Unable to load goals. Please try refreshing the page.';
      ...
      if (typeof showToast === 'function') {
        showToast('Error loading goals. Please check your connection.', 'error');
      }
    }
  }
  ...
  async function editGoal(goalId) {
    try {
      ...
      if (window.openSetGoalModal) {
        ...
        console.warn('Goal modal function not available');
        if (typeof showToast === 'function') {
          showToast('Goal modal is not available. Please refresh the page.', 'error');
        } else {
          alert('Goal modal will be available when modals.js is loaded');
        }
      }
  ...
  async function initGoalsPage() {
    ...
    console.log('Authentication verified:', authState.userData?.role || 'player');
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    try {
      await initGoalsPage();
      // Enable header compression once page is initialized
      try {
      ...
      if (container) {
        showEmptyState('goalsContainer', {
          ...
          message: 'Unable to load the goals page. Please refresh and try again.',
  ...
  window.addEventListener('authStateChanged', async (event) => {
    ...
    if (user && userData) {
      ...
      const container = document.getElementById('goalsContainer');
      if (container && container.innerHTML.includes('Authentication Required')) {
